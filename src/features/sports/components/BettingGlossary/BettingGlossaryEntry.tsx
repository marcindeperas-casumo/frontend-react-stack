...
export class BettingGlossaryEntry extends React.PureComponent<Props> {
    scrollToTerm = (linkedElement: HTMLElement) => {
        const highlighted = window.document.querySelectorAll(`.${highlightedClass}`) || [];
        ...
        if ("scrollIntoView" in linkedElement) {
            ...
            const topBarOffset = parseInt(getCssCustomProperty("--shell-offset-top") || "75px", 10);
            window.document
                .querySelector(".c-modal__content")
                .scrollTo(0, (linkedElement as any).offsetTop + topBarOffset - window.innerHeight * 0.5);
        }
    };
    ...
    handleLinkedEntries = (event: SyntheticEvent<HTMLElement>) => {
        ...
        const linkedElement = document.querySelector(`[${dataAttr.term}="${linkedTermId}"]`);
        if (linkedElement) {
    ...
    render() {
        return (<Flex.Item className="c-betting-glossary-entry t-color-grey-90 u-font-weight-normal u-pointer-events-none" onClick={this.handleLinkedEntries} data-glossary-term={this.props.id}>
        <strong>{this.props.term}: </strong>
        {!isCmsEntryEmpty(this.props.aka) && (<span>
            <em>
              <DictionaryTerm data-test-glossary-entry-aka termKey="glossary.aka"/>{" "}
              {this.props.aka}
              <br />
            </em>
          </span>)}
