...
import { kambiClientEventHandler, KAMBI_EVENTS, } from "./kambiClientEventHandler";
...
export default class KambiClient extends React.Component<Props, State> {
    ...
    onNotification = (event: {
        ...
        if ((event as any).name === "loginRequestDone") {
            this.props.onLoginCompleted && this.props.onLoginCompleted();
        }
        const isFirstBet = (event as any).data?.event === KAMBI_EVENTS.PLACE_BET && this.state.sportsFirstBet;
        if (isFirstBet) {
        ...
        if ((event as any)?.data?.event === "kambi betslip status") {
            const isBetSlipMaximized = (event as any).data.kambi.betslip.position === "maximized";
    ...
    onWidgetMessage = (message: MessageEvent) => {
        ...
        if (message.data.type === "SHOW_TERMS") {
            getKambiWidgetAPI().then(wapi => {
                // @ts-expect-error ts-migrate(2339) FIXME: Property 'set' does not exist on type 'KambiWidget... Remove this comment to see the full error message
    ...
    componentWillUnmount() {
        (window as any)._kbc && (window as any)._kbc.dispose();
    ...
    redirectToUserHomeRoute = (prevHomeRoute: ?string) => {
        ...
        const onPrevHomeRoute = prevHomeRoute && window.location.hash === `#${prevHomeRoute}`;
    ...
    replaceSportsHub = () => {
        if (window.location.hash.indexOf("sports-hub") > -1) {
            // eslint-disable-next-line fp/no-mutation
            window.location.hash = window.location.hash.replace("sports-hub", "filter");
        }
    };
    handleHashChange = () => {
        // @ts-expect-error ts-migrate(2554) FIXME: Expected 1 arguments, but got 0.
    ...
    render() {
        return (<div className={classNames("t-background-grey-0", {
            "c-kambi-client--hidden": this.props.isHidden,
        })}>
        <div id="KambiBC" className="u-padding-top--md@desktop u-padding-top--sm@tablet u-padding-top--none@mobile"/>

        {this.props.isBetslipVisible ? null : (<style dangerouslySetInnerHTML={{
            __html: `
