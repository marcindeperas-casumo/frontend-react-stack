...
import { bridgeFactory, generateColumns, makeProtocolAwareUrl, matchingGroups, renderBets, commaSeparated, createReducer, formatCurrency, getSymbolForCurrency, interpolate, interpolateWithJSX, isCmsEntryEmpty, findOr, convertHoursToDaysRoundUp, convertTimestampToLuxonDate, interpolateTimeInterval, canBeInterpolated, formatTime, timeRemainingBeforeStart, isTestEnv, isIosNative, getAppVersion, convertLuxonDurationObjectToSeconds, addPointerEventStylesToLinkElements, decodedUrlParams, bonusBalanceDisplay, hasAlphaCharactersOnly, } from "./utils";
...
describe("Native app related functions", () => {
    ...
    test("getAppVersion", () => {
        ...
        expect(getAppVersion()).toBe(`ios/{${(window as any).native.version}}`);
        // eslint-disable-next-line fp/no-delete
        delete (window as any).native;
...
describe("findOr()", () => {
    ...
    test("should return the first item that satisfies the predicate", () => {
        ...
        [result1, result2, result3].forEach(result => expect(result).toEqual({ c: 3 }));
    });
    test("should return the default item if no items satisfy the predicate", () => {
        ...
        const result2 = findOr(defaultValue, predicate)(items);
        const result3 = findOr(defaultValue)(predicate)(items);
        [result1, result2, result3].forEach(result => expect(result).toEqual(defaultValue));
    });
});
...
describe("makeProtocolAwareUrl()", () => {
    ...
    test("should not touch it otherwise", () => {
        expect(makeProtocolAwareUrl("http://casumo.com/cometd")).toBe("http://casumo.com/cometd");
    });
});
...
describe("interpolate()", () => {
    test("should replace dynamic strings", () => {
        ...
        expect(interpolate(input, { var: "variable", something: "a value" })).toBe(output);
    });
...
describe("interpolateWithJSX()", () => {
    test("should replace with components", () => {
        ...
        expect(mount(<div>{interpolateWithJSX({ foo: <Component /> }, input)}</div>).text()).toBe(output);
    });
});
describe("isCmsEntryEmpty()", () => {
    test("should return true for values that should be handled as empty", () => {
        ["empty", "EmPtY", "", null, undefined].map(value => expect(isCmsEntryEmpty(value)).toBe(true));
    });
    test("should return false for non-empty values", () => {
        ["not-empty", "!empty", " "].map(value => expect(isCmsEntryEmpty(value)).toBe(false));
    });
});
describe("convertHoursToDays()", () => {
    ...
    describe("formatTime()", () => {
        test("should properly format Unix time in millis as localized 24-hour time with seconds", () => {
            expect(formatTime(1576758921344)).toEqual(expect.stringMatching(/^\d\d:35:21/));
        });
    });
    ...
    describe("convertLuxonDurationObjectToSeconds()", () => {
        ...
        test("should return 172980 if given { days: 2, minutes: 3 }", () => {
            expect(convertLuxonDurationObjectToSeconds({ days: 2, minutes: 3 })).toEqual(172980);
        });
    });
...
describe("bonusBalanceDisplay to show bonus balance in different forms", () => {
    test("should show EUR bonus balance amount followed with the word bonus", () => {
        const bonusBalanceDisplayLongText = bonusBalanceDisplay(100, "EUR", "Bonus", "en-en");
        expect(bonusBalanceDisplayLongText).toMatch("Bonus");
    });
    test("should show GBP bonus balance without the word bonus", () => {
        const bonusBalanceDisplayLongText = bonusBalanceDisplay(200.2, "GBP", "Bonus", "en-GB", true);
        expect(bonusBalanceDisplayLongText).toMatch("+£");
    });
    test("should receive '' if no value is passed", () => {
        const bonusBalanceDisplayLongText = bonusBalanceDisplay(0, "EUR", "", "en-en");
        expect(bonusBalanceDisplayLongText).toMatch("");
    });
});
