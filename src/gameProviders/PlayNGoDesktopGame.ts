...
export class PlayNGoDesktopGame extends BaseGame {
        ...
        if (url) {
            injectScript(url).then(() => {
                ...
                if (this.engage) {
                    ...
                    this.engage.addEventListener(EVENTS.ON_GAME_ROUND_START.type, this.setGameAsActive.bind(this));
                    this.engage.addEventListener(EVENTS.ON_GAME_ROUND_END.type, this.setGameAsIdle.bind(this));
                }
            });
        }
    }
    onUnmount() {
        ...
        if (this?.engage) {
            ...
            this.engage.request(COMMANDS.END_GAME);
            this.engage.removeEventListener(EVENTS.ON_GAME_ROUND_START.type, this.setGameAsActive.bind(this));
            this.engage.removeEventListener(EVENTS.ON_GAME_ROUND_END.type, this.setGameAsIdle.bind(this));
        }
    }
    pauseGame() {
