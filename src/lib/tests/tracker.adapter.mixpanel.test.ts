...
describe("Lib/Tracker.Adapter.Mixpanel", () => {
    ...
    test("initialises mixpanel when creating the adapter", () => {
        ...
        expect((window as any).mixpanel.init).toHaveBeenCalledTimes(1);
        expect((window as any).mixpanel.init).toHaveBeenCalledWith(mixpanelToken, {}, mixpanelProjectName);
    });
    describe("track()", () => {
        test("tracks an event with mixpanel", () => {
            const eventName = "event";
            const data = { foo: "bar" };
            const spy = (window as any).mixpanel[config.mixpanelProjectName].track;
    ...
    describe("setState()", () => {
        test("registers super properties for mixpanel events", () => {
            const data = { foo: "bar" };
            const spy = (window as any).mixpanel[config.mixpanelProjectName].register;
