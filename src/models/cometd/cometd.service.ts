...
export const CometdFactory = ({ cometd, url }) => {
    ...
    async function subscribe(channel, callback, ...args) {
        ...
        if (!subscriptions[channel]) {
            subscriptions[channel] = await cometd.subscribe(channel, x => (subscriptionCallbacks[channel] || []).forEach(fn => fn(x)), ...args);
        }
    }
    ...
    function emit(channel, data) {
        ...
        const callCallback = callback => callback.forEach(fn => fn({ data, channel }));
