...
import { handshakeSelector, applicationHandshakeSelector, isApplicationHandshakeLoaded, session, playersSelector, isAuthenticated, playerIdSelector, sessionIdSelector, playerSelector, countrySelector, currencySelector, walletAmountSelector, bonusAmountSelector, marketSelector, languageSelector, getCmsHash, hasMadeFirstDepositSelector, optedInReelRacesSelector, adventureLevelsSelector, localeSelector, tournamentChannelsSelector, walletIdSelector, playerNameSelector, socialSecurityNumberSelector, isSuspiciousAccount, verticalSelector, featureFlagSelector, playerCasumoNameSelector, jurisdictionSelector, registrationDateSelector, complianceStatePropertySelector, isProductionBackendSelector, piqConfigSelector, } from "./handshake.selectors";
describe("Handshake selectors", () => {
    ...
    describe("verticalSelector()", () => {
        ...
        test("returns SPORTS if player has a sports welcome offer", () => {
            const state = createHandshakeStateWithWelcomeOfferId("wo-sports-this-part-doesnt-mattter");
    ...
    describe("complianceStatePropertySelector()", () => {
        ...
        test("should return object for DGA compliance state property", () => {
            expect(complianceStatePropertySelector(COMPLIANCE_STATE_PROPERTY.DGA)(state)).toEqual({ prop: "value" });
        });
        test("should return 'any value' for AML complaince state property", () => {
            expect(complianceStatePropertySelector(COMPLIANCE_STATE_PROPERTY.AML)(state)).toEqual("any value");
        });
    });
    ...
    test("piqConfig", () => {
        ...
        expect((piqConfigSelector(state) as any).apiUrl).toEqual("some_url");
    });
});
