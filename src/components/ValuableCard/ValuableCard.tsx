...
export class ValuableCard extends PureComponent<Props> {
  ...
  isXmasValuable() {
    const { valuableBadgeName } = this.props;
    if (valuableBadgeName) {
      return (
        valuableBadgeName.startsWith("christmas21") ||
        valuableBadgeName.startsWith("xmas21")
      );
    }
    return false;
  }
  ...
  get maskedImage() {
    ...
    return (
      <MaskImage
        ...
        imgixOpts={
          this.isValuableTypeSpins && !this.isXmasValuable()
            ? imgixOptsForSpins
            : {}
        }
      >
