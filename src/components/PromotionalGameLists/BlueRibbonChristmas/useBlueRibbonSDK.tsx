...
import { currencySelector, playerIdSelector, isProductionBackendSelector, } from "Models/handshake";
...
export function useBlueRibbonSDK() {
    ...
    const blueRibbonConfig = {
        ...
        loginAnonymousPlayer: () => {
            return fetch(urls.loginAnonymous, { method: "POST" }).then(raw => raw.json());
        },
        loginAuthenticatedPlayer: () => {
            return fetch(urls.loginAuthenticated, { method: "POST" }).then(raw => raw.json());
        },
    };
...
export function useBlueRibbonSDKAnonymous() {
    ...
    React.useEffect(function connectBlueRibbonSDK() {
        if (!sdk) {
            return;
        }
...
export function useBlueRibbonAutoOptIn() {
    ...
    React.useEffect(() => {
        ...
        fetch(urls.handshake)
            .then(raw => raw.json())
            // @ts-expect-error ts-migrate(2532) FIXME: Object is possibly 'undefined'.
            .then(({ externalPlayerReference }) => sdk.connect({ currency, playerId: externalPlayerReference }))
            .then(async () => {
            const res: {
                gamesDetails: Array<{
                    gameId: string;
...
export function usePotStateChangeEvent() {
    ...
    React.useEffect(() => {
        ...
        sdk.events.on(BlueRibbon.constants.events.POT_STATE_CHANGED_EVENT, (x: PotStateChangeEvent) => {
            setPots(oldPots => ({
                ...oldPots,
