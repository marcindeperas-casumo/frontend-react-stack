...
// @ts-expect-error ts-migrate(2305) FIXME: Module '"./reducer"' has no exported member 'type'... Remove this comment to see the full error message
import { type, TBoostersState, reducer } from "./reducer";
import { getBoostersConfig, getInitialState, UPDATE_ANIMATION, UPDATE_PREV_VALUES, } from "./const";
...
export const ReelRaceBoosterPoints = ({ bigWins = 0, megaWins = 0, triples = 0, wins = 0, }: Props) => {
        ...
        for (const booster in boosters) {
            ...
            if (currentBooster && currentBooster !== state.boosters[booster]) {
                ...
                if (typeof pointsContainer.addEventListener === "function") {
                    pointsContainer.addEventListener("animationend", () => {
                        dispatch({
    ...
    return (<div className="c-reel-race-icon__boost-points o-position--absolute" ref={pointsContainerRef}>
      <ReelRaceBoosterPointsValue animating={state.animation.active} basePoints={state.animation.basePoints} extraPoints={state.animation.extraPoints}/>
    </div>);
};
