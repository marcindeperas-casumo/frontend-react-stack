...
import { types, initFetch, shouldFetchPageFactory } from "Reducers/curated";

export function* fetchPageSaga(action) {
  const shouldFetch = yield select(shouldFetchPageFactory());

  // init fetch only if page not in store
  if (shouldFetch) {
    yield put(initFetch());
